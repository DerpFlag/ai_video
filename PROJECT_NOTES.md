...
- **TTS (Qwen Space)**:
  ...
  - **Why “Audio URL not found” / “Stream tail: null”**: The Space sends **heartbeat** events with `data: null`. Sometimes for **voice clone** it never sends `event: complete` (only heartbeats), so we never see a URL—often after 2–3 segments when the Space is under load. Fixes: (1) We now **ignore `data: null`** and keep reading until we get a URL or the stream ends. (2) We accept URL from **`event: complete`** and from **nested shapes** (e.g. `data.data`, `data.output`). (3) **Longer delays**: 5s between normal segments, **6s between voice-clone segments**, 10s between retries. (4) On failure we log the **raw stream tail** (last ~900 chars) so you see exactly what the Space returned (e.g. only `event: heartbeat data: null` = Space never completed). **Test script**: Run `node scripts/test-qwen-tts-stream.mjs 3` to call the Space 3 times with 5s delay and print the raw SSE (no Supabase); use to verify Space behaviour.
